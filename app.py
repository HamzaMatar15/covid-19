...
if not st.session_state.authenticated:
    ...
    with col2:
        ...
        st.markdown("---")
        pwd = st.text_input("Enter dashboard password",
        ...
        st.markdown(
            "<br><small style='color:#555'>Consultant tool - authorized access only</small>", unsafe_allow_html=True)
    st.stop()
...
def load_data():
    ...
    latest["cfr"] = (latest["total_deaths"] /
                     latest["total_cases"] * 100).round(2)
...
if len(date_range) == 2:
    start_date, end_date = pd.Timestamp(
        date_range[0]), pd.Timestamp(date_range[1])
else:
...
if page == "Overview":
    ...
    st.markdown("<p class='section-sub'>Global COVID-19 summary - cases, deaths, and trends over time</p>",
                unsafe_allow_html=True)

    # KPI Cards
    total_cases = filtered_latest["total_cases"].sum()
    total_deaths = filtered_latest["total_deaths"].sum()
    total_vacc = filtered_latest["people_vaccinated"].sum(
    ) if "people_vaccinated" in filtered_latest.columns else 0
    ...
    with col1:
        st.markdown(
            "<p class='section-title'>Daily new cases over time (7-day smoothed)</p>", unsafe_allow_html=True)
    ...
    with col2:
        st.markdown("<p class='section-title'>Cases by continent</p>",
                    unsafe_allow_html=True)
        cont_data = (
    ...
    # Top 10 countries
    st.markdown("<p class='section-title'>Top 10 countries by total cases</p>",
                unsafe_allow_html=True)
    ...
    top10 = filtered_latest.nlargest(10, top10_metric)[
        ["location", "continent", top10_metric]].dropna()
    fig_bar = px.bar(
        top10.sort_values(top10_metric), x=top10_metric, y="location",
        orientation="h", color="continent",
        color_discrete_sequence=px.colors.qualitative.Set2,
        labels={top10_metric: top10_metric.replace(
            "_", " ").title(), "location": ""}
    )
...
#
    ...
    st.markdown("<p class='section-sub'>Country-level distribution of COVID-19 burden worldwide</p>",
                unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(
            "<p class='section-title'>Top 15 worst-affected countries</p>", unsafe_allow_html=True)
        top15 = filtered_latest.nlargest(15, map_metric)[
            ["location", map_metric]].dropna()
        fig_t15 = px.bar(
            top15.sort_values(map_metric), x=map_metric, y="location",
            orientation="h", color=map_metric,
            color_continuous_scale="Reds",
            labels={map_metric: map_metric.replace(
                "_", " ").title(), "location": ""}
        )
    ...
    with col2:
        st.markdown("<p class='section-title'>Continental summary</p>",
                    unsafe_allow_html=True)
        cont_sum = (
            filtered_latest.groupby("continent")[
                ["total_cases", "total_deaths"]]
            .sum().reset_index()
        )
        cont_sum["cfr"] = (cont_sum["total_deaths"] /
                           cont_sum["total_cases"] * 100).round(2)
        fig_cont = px.bar(
...
#
    ...
    with col1:
        st.markdown(
            "<p class='section-title'>Median age vs. case fatality rate</p>", unsafe_allow_html=True)
        scatter_data = filtered_latest[[
            "location", "continent", "median_age", "cfr", "total_cases"]].dropna()
        fig_sc = px.scatter(
    ...
    with col2:
        st.markdown(
            "<p class='section-title'>Population aged 65+ vs. death rate per million</p>", unsafe_allow_html=True)
        aged_data = filtered_latest[[
            "location", "continent", "aged_65_older", "total_deaths_per_million"]].dropna()
        fig_aged = px.scatter(
    ...
    st.markdown("<p class='section-title'>Risk factor comparison by continent</p>",
                unsafe_allow_html=True)
    ...
    fig_risk = px.bar(
        ...
        color="continent", color_discrete_sequence=px.colors.qualitative.Set2,
        labels={risk_metric: risk_metric.replace(
            "_", " ").title(), "continent": ""}
    )
    ...
    st.markdown("<p class='section-title'>Male vs. female smokers by continent</p>",
                unsafe_allow_html=True)
    smoke_data = (
        filtered_latest.groupby("continent")[
    ...
    smoke_melt = smoke_data.melt(
        id_vars="continent", var_name="Gender", value_name="Smokers (%)")
    smoke_melt["Gender"] = smoke_melt["Gender"].map(
...
#
    ...
    st.markdown("<p class='section-sub'>Vaccine rollout progress and its relationship with COVID-19 outcomes</p>",
                unsafe_allow_html=True)

    VACC_COL = "people_vaccinated_per_hundred"
    has_vacc = VACC_COL in filtered_df.columns and filtered_df[VACC_COL].notna(
    ).sum() > 0
    ...
    if not has_vacc:
        ...
        st.markdown(
            "<p class='section-title'>Testing effort over time (proxy for healthcare response)</p>", unsafe_allow_html=True)

        available_countries = sorted(
            filtered_df[filtered_df["new_tests_smoothed"].notna()
                        ]["location"].unique()
        )
        default_countries = ["United States",
                             "United Kingdom", "France", "Brazil", "India"]
        default_countries = [
            c for c in default_countries if c in available_countries][:5]
        ...
        if selected_countries:
            ...
            fig_test = px.line(
                ...
                color_discrete_sequence=px.colors.qualitative.Set2,
                labels={
                    "new_tests_smoothed": "New Tests (7-day avg)", "date": "Date", "location": "Country"}
            )
        ...
        with col1:
            st.markdown(
                "<p class='section-title'>Tests per case by continent (positivity proxy)</p>", unsafe_allow_html=True)
            tpc = (
        ...
        with col2:
            st.markdown(
                "<p class='section-title'>Positive rate by continent</p>", unsafe_allow_html=True)
            pr = (
            ...
            fig_pr = px.bar(
                ...
                color_discrete_sequence=px.colors.qualitative.Set2,
                labels={
                    "positive_rate": "Positive Rate (avg)", "continent": ""}
            )
        ...
        st.markdown(
            "<p class='section-title'>Hospital beds per 1,000 people vs. death rate</p>", unsafe_allow_html=True)
        hb_data = filtered_latest[[
            "location", "continent", "hospital_beds_per_thousand", "total_deaths_per_million"]].dropna()
        fig_hb = px.scatter(
        ...
        if selected_countries:
            st.markdown(
                "<p class='section-title'>Vaccination rollout over time</p>", unsafe_allow_html=True)
            vacc_ts = filtered_df[
                filtered_df["location"].isin(selected_countries) &
            ...
            fig_vacc = px.line(
                ...
                color_discrete_sequence=px.colors.qualitative.Set2,
                labels={
                    VACC_COL: "People Vaccinated (%)", "date": "Date", "location": "Country"}
            )
        ...
        with col1:
            st.markdown(
                "<p class='section-title'>Vaccination rate vs. death rate per million</p>", unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(
                "<p class='section-title'>Top 10 most vaccinated countries</p>", unsafe_allow_html=True)
            top_vacc = (
...
#
    ...
    if len(selected_features) < 1:
        ...
        if len(model_data) < 10:
            st.warning(
                "Not enough data to train the model. Try adding more features or adjusting filters.")
        else:
            ...
            col1.metric(
                "R  Score", f"{r2:.3f}", help="How well the model explains variation in CFR (1.0 = perfect)")
            ...
            with col_a:
                st.markdown(
                    "<p class='section-title'>Predicted vs. actual CFR</p>", unsafe_allow_html=True)
            ...
            with col_b:
                st.markdown(
                    "<p class='section-title'>Feature importance (coefficients)</p>", unsafe_allow_html=True)
            ...
            st.markdown(
                "<p class='section-title'>Live CFR predictor - adjust inputs to get a prediction</p>", unsafe_allow_html=True)
            st.markdown(
                "<p class='section-sub'>Simulate a country's predicted case fatality rate based on its health indicators</p>", unsafe_allow_html=True)
